...
def error(errorLabel):
	sys.stderr.write("{prefix} error: {label}\n".format(prefix=_progName, label=errorLabel))
	
	error_message = QtWidgets.QMessageBox()
	error_message.setWindowTitle('{} error !'.format(_progName))
	error_message.setIcon(QtWidgets.QMessageBox.Critical)
	error_message.setInformativeText(errorLabel)
	error_message.exec_()
	sys.exit(2)
...
# Class : spif
#
# An object that represents contents of a spif
#
class spif():
   """
	   Object that represents a Security Policy Information File
   """
   def __init__(self, fileName = "/etc/spifmgr/default.spif"):

        debug("Parsing {file}".format(file=fileName))
        try:
   ...
   def getGeneralInfo(self):
      ...
      self.generalInfo = {}
      debug("Loading securityPolicyId")
	   
      if self.root.find("spif:securityPolicyId",ns) is None: error('No security policy id in template')
...
class SpifMgr(QtWidgets.QMainWindow):
    # INIT
    def __init__(self, spif, window_width=1000, window_height=950):
        debug("Displaying {file}".format(file=str(spif)))
        super(SpifMgr, self).__init__()
...
#        self.file_name = 'Untitled.txt'
        self.file_name = path.basename(str(spif))
        self.file_path = ['/', '']
#        self.file_type = self.basename.split('.')[-1]
...
#        self.setWindowIcon(QtGui.QIcon('assets/icons/notepad.png'))
    ...
    def notepad_ui(self):
        ...
        self.new_file()
        self.finder_focus()
        self.setWindowTitle('{} - SPIF Manager'.format(os.path.basename(self.file_name)))
...
#            self.file_name = 'Untitled'
            self.setWindowTitle("{} - SPIF Manager".format(self.file_name))
            self.need_saving(False)
...
#            syntax_lbl.setText('Default')
    ...
    def save_box(self, new=False, open=False):
        ...
        reply = QtWidgets.QMessageBox().question(self, 'SPIF Manager', "Do you want to save {}".format(self.file_name),
        ...
        try:
            if self.file_path:
                if (self.file_path[0].split('/')[-1].lower()) == self.file_name.lower():
                    ...
                    self.setWindowTitle("{} - SFIF Manager".format(self.file_name))
                    with file_open:
                        file_open.write(self.text_widget.toPlainText())
                        self.need_saving(False)
                    return
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            debug("Debug mode set")
             ...
             debug("Loading {}".format(arg))
             currentFile = str(arg)
             
currentSpif = spif() if (currentFile is None) else spif(currentFile)

currentSpif.getGeneralInfo()
notes = SpifMgr(currentSpif)
